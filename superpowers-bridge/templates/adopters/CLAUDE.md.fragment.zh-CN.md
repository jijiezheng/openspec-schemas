...
<!-- 将此部分复制到项目 CLAUDE.md 中，以便 Claude 正确路由后续工作。-->
<!-- 如有定制可修改 schema 名称和桥接仓库 URL；否则保持不变。-->

## 工作流路由（会话启动时阅读）

本仓库使用 [`superpowers-bridge`](https://github.com/JiangWay/openspec-schemas/tree/main/superpowers-bridge) 桥接 OpenSpec 与 Superpowers。集成规则（语言、制品路径、PRECHECK）遵循该桥接的 README；本节为 Claude 提供路由指引。
...
详细说明：[superpowers-bridge README §Entry & exit gates](https://github.com/JiangWay/openspec-schemas/blob/main/superpowers-bridge/README.md#entry--exit-gates)。
